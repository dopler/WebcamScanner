...
def main():
    ...
    cv2.createTrackbar("Threshold1", "Parameters", 138, 255, empty)
    ...
    while True:
        ...
        cropPts = getContours(imgDil, imgContour)
        imgInvert = cv2.bitwise_not(imgCanny)

        imgStack = stackImages(0.8, ([img, imgGray, imgCanny],
                                     [imgDil, imgContour, imgInvert]))
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            print(*cropPts)
            cropImage = imgInvert[cropPts[1]:(cropPts[1] + cropPts[3]), cropPts[0]:(cropPts[0] + cropPts[2])]
